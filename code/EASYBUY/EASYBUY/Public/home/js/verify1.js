...
jQuery(function(){
    ...
    $("#username").blur(function(){
        ...
        var o=$("#username");
        if(!v){
            showInfo($("#username_msg").css('color','red'),"用户名不能为空",'err');

            setTimeout(function(){o.focus()},0);

            return false;
        }else if(len<5||len>25){
            showInfo($("#username_msg").css('color','red'),'5-25个字符','err');
            setTimeout(function(){o.focus()},0);
            return false;
        }else if(isNum){
            showInfo($("#username_msg").css('color','red'),'用户名不能以纯数字组成','err');
            setTimeout(function(){o.focus()},0);
            return false;
        }else if(isName){
            showInfo($("#username_msg").css('color','red'),'用户名不能有特殊字符存在','err');
            setTimeout(function(){o.focus()},0);
            return false;
        }else{
            showInfo($("#username_msg"),'','ok');
    ...
    $("#repswd").blur(function(){
        ...
        if (v==0||c==0)
        {
            showInfo($("#repswd_msg").css('color','red'),'密码和确认密码不能为空','err');
            setTimeout(function(){o.focus()},0);
            return false;
        }
        else
        {
            if( v !== c){
                showInfo($("#repswd_msg").css('color','red'),'两次密码不相同','err');
                setTimeout(function(){o.focus()},0);
                return false;
            }else{
    ...
    $("#phone").blur(function(){
        ...
        if(!u) {
            ...
            if(isPhonedate == 0){
                ...
                showInfo($("#phone_msg").css('color','red'),'手机号码格式输入错误,正确的格式:13000000000','err');
                setTimeout(function(){o.focus()},0);
                return false;
            }
            else{
